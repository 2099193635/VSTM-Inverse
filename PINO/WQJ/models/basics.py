...
class SpectralConv1d_filter(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, lowpass_fc):
        super(SpectralConv1d_filter, self).__init__()
    ...
    def forward(self, x):
        ...
        # Compute Fourier coeffcients up to factor of e^(- something constant)
        x_ft = torch.fft.rfftn(x, dim=[2])
        freq = torch.linspace(0, 1, x_ft.size(-1))
        # print(freq.is_cuda)
        mask = torch.where(freq <= self.lowpass_fc, torch.tensor(1.0), torch.tensor(0.0))
        mask = mask.to(torch.device('cuda:0'))
        freq = freq.to(torch.device('cuda:0'))
        # print(freq.is_cuda,mask.is_cuda)
        # loss_BC = torch.tensor([0.0]).to(torch.device('cuda:0'))
        x_ft_filtered = x_ft * mask.unsqueeze(0).unsqueeze(0) 
        
        # x_ft_filtered = self.lowpass_filter(x_ft, self.lowpass_fc)

        # Multiply relevant Fourier modes
        out_ft = torch.zeros(batchsize, self.in_channels, x.size(-1)//2 + 1, device=x.device, dtype=torch.cfloat)
        out_ft[:, :, :self.modes1] = compl_mul1d(x_ft_filtered[:, :, :self.modes1], self.weights1)

        # Return to physical space
        x = torch.fft.irfft(out_ft, n=x.size(-1), dim=2)
        return x
    
    def lowpass_filter(self, x_ft, lowpass_fc):
        # Perform low-pass filtering in the frequency domain
        # x_ft: input Fourier coefficients
        # lowpass_fc: cutoff frequency for low-pass filtering

        freq = torch.linspace(0, 1, x_ft.size(-1))  # Frequency values from 0 to 1
        mask = torch.where(freq <= lowpass_fc, torch.tensor(1.0), torch.tensor(0.0))  # Create a binary mask for low-pass filtering
        print(freq.is_cuda,mask.is_cuda)
        print(x_ft.is_cuda)

        x_ft_filtered = x_ft * mask.unsqueeze(0).unsqueeze(0)  # Apply the mask to the Fourier coefficients

        return x_ft_filtered
