"""
train_deeponet_physics.py
DeepONet (Branch=FNO + Trunk=MLP) + VTCM 物理信息训练脚本。
与 train_pino_physics_informed.py 结构一致，仅模型替换为 VTCMDeepONet。
"""
import argparse
import os
import sys
...
from utils import (
    ...
    EarlyStopping, physics_weight_at_step, deriv_mix_alpha_at_step,
...
def parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="DeepONet + VTCM Physics-Informed Training")
    # 训练超参
    ...
    # 物理损失调度
    p.add_argument("--physics_weight",       type=float, default=0.5)
    p.add_argument("--warmup_steps",         type=int,   default=5)
    p.add_argument("--physics_ramp_steps",   type=int,   default=10)
    p.add_argument("--deriv_blend_steps",    type=int,   default=2000)
    p.add_argument("--pred_deriv_max_ratio", type=float, default=0.0)
    p.add_argument("--nexus_physics_weight", type=float, default=0.1)
    # 数据损失
    ...
    # 路径
    p.add_argument("--ckpt_dir",    type=str, default=str(_DIR / "checkpoints_deeponet_physics"))
    p.add_argument("--fig_dir",     type=str, default=str(_DIR / "figures_deeponet_physics"))
...
# ── 验证 ──────────────────────────────────────────────────────────────────────
def validate(model, loader, device, out_norm_stats, vtcm_ctx, args,
             global_step: int, epoch: int) -> dict:
    model.eval()
    data_sum, pde_sum, n = 0.0, 0.0, 0
    comp_sums = {k: 0.0 for k in ("car_body", "bogie_front", "bogie_rear",
                                   "nexus_w1", "nexus_w2", "nexus_w3", "nexus_w4")}
    viz_pred = viz_tgt = None
    use_physics = global_step >= args.warmup_steps
    pw = physics_weight_at_step(global_step, args.warmup_steps,
                                args.physics_ramp_steps, args.physics_weight)
    da = deriv_mix_alpha_at_step(global_step, args.warmup_steps,
                                 args.deriv_blend_steps, args.pred_deriv_max_ratio)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if use_physics:
                lp, comps = vtcm_physics_loss(pred, batch, device, out_norm_stats,
                                              vtcm_ctx, da, args.nexus_physics_weight)
                for k in comp_sums:
                    comp_sums[k] += float(comps[k].item())

            data_sum += ld.item(); pde_sum += lp.item(); n += 1
            if viz_pred is None:
                viz_pred = pred[0].detach().cpu().numpy()
                viz_tgt  = target[0].detach().cpu().numpy()

    avg_data  = data_sum / max(n, 1)
    avg_pde   = pde_sum  / max(n, 1)
    avg_total = avg_data + pw * avg_pde
    ...
    return {
        "total": avg_total, "data": avg_data, "pde": avg_pde,
        "components": {k: v / max(n, 1) for k, v in comp_sums.items()},
    }
...
def main():
    ...
    vtcm_ctx = build_vtcm_context(vx_ms, device=device)

    print(f"[DeepONet+Physics] Device={device}  train={len(train_ds)}  val={len(val_ds)}"
          f"  in_ch={in_ch}  out_ch={out_ch}  vx={vx_ms:.1f} m/s")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in pbar:
            ...
            if global_step >= args.warmup_steps:
                lp, _ = vtcm_physics_loss(pred, batch, device, out_norm_stats,
                                          vtcm_ctx, da, args.nexus_physics_weight)
            ...
            pbar.set_postfix(loss=f"{loss.item():.3e}", data=f"{ld.item():.3e}",
                             pde=f"{lp.item():.3e}", pw=f"{pw:.2e}")

        avg_train = epoch_loss / max(n_upd, 1)

        val_m = {"total": 0.0, "data": 0.0, "pde": 0.0, "components": {}}
        if val_loader is not None:
            val_m = validate(model, val_loader, device, out_norm_stats,
                             vtcm_ctx, args, global_step, epoch)

        pw_ep = physics_weight_at_step(global_step, args.warmup_steps,
                                       args.physics_ramp_steps, args.physics_weight)
        print(f"[{epoch + 1:4d}/{args.epochs}] train={avg_train:.4e}"
              f"  val={val_m['total']:.4e} (data={val_m['data']:.3e} pde={val_m['pde']:.3e})"
              f"  pw={pw_ep:.2e}  lr={optimizer.param_groups[0]['lr']:.3e}")
        c = val_m.get("components", {})
        if c:
            print(f"  pde: car={c.get('car_body',0):.2e}"
                  f" bf={c.get('bogie_front',0):.2e} br={c.get('bogie_rear',0):.2e}"
                  f" n1-4={c.get('nexus_w1',0):.2e}/{c.get('nexus_w2',0):.2e}"
                  f"/{c.get('nexus_w3',0):.2e}/{c.get('nexus_w4',0):.2e}")
